---
title: "Early Course Evaluation - STA 418/518"
...
# DO NOT RUN
# 
# sta518_evals <- googlesheets4::read_sheet("1N1FjCUVMrn4gx958wnCKLkLGPi5mrtpUCD4aIl_RsaE")
#
...
```{r best-data, eval=TRUE, message = FALSE}
best <- sta518_evals %>% 
...
```{r change-plot, eval=TRUE, fig.height=6}
change_cleaned %>%
  count(bigram, sort = TRUE) %>%
  filter(n > 2) %>% 
  ggplot(aes(label = bigram, size = n)) + 
...
```{r improve-data, eval=TRUE, message = FALSE}
...
```{r improve-plot, eval=TRUE, fig.height=6}
improve_cleaned %>%
  count(bigram, sort = TRUE) %>%
  filter(n > 2) %>% 
  ggplot(aes(label = bigram, size = n)) + 
